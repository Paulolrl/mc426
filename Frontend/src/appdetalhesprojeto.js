...
export default class AppDetalhesProjeto extends Component {
  ...
  async handleSubmit() {
      ...
      let response1 = await fetch(apiUrl + "/projetos/" +this.state.idProjeto, {
          method: 'POST',
          headers: {
      ...
      if(this.state.idEquipes != ""){
          ...
          let response2 = await fetch(apiUrl+"/projetos/"+responseJson.id+"/equipes/",{
              method: 'POST',
              headers: {
                  'Authorization':'Basic '+ authorizationBasic
              },
  ...
  componentDidMount() {
    ...
    fetch(apiUrl + /projetos/ + this.state.idProjeto, {
      method: 'GET',
      headers: {
        'Authorization': 'Basic ' + authorizationBasic,
